...
class Postos{
	...
	function BuscarDadosdoFilhoePai($idposto, $idprocesso=null, $debug=null, $listar = "Lista")
	{
		...
		// BUSCANDO DADOS DO FILHO
                ...
                {
                    case ("Lista"):
                        $comp = "inner join postos_campo_lista pcl ON (pcl.idpostocampo = pc.id and pcl.idposto = wp.id)";
                    break;
                }
                
                $sql = "SELECT pc.campo, w.valor, w.idprocesso, p.idpai, wt.id idworkflowtramitacao, p.status, pc.id idcampo
                        FROM  processos p 
                            INNER JOIN workflow_dados w ON (w.idprocesso = p.id)
                            INNER JOIN  postos_campo pc ON (pc.id = w.idpostocampo and w.idpostocampo = pc.id)
	...
	function getLista($app,   $idposto ){
		...
		$array = $this->BuscarDadosdoFilhoePai($idposto, null, 1);
